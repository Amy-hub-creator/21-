...
void gesture_systematic_error_calc(void)
{
    ...
    for(uint16 i = 0; i < imu_parameters->sampling_times; i++)
    {
        ...
        imu660rb_get_gyro();
        // 积分
        imu_temp->gyro_integral_z += (float)imu660rb_gyro_z;
...
void yaw_integral_calc(void)
{
    if(glb_flag->Offset_flag == 1 && glb_flag->yaw_flag == 1)
    {
        ...
        // 去零漂
        imu_temp->sampling_data = imu660rb_gyro_z - imu_temp->gyro_integral_z;
...
float Angle_X_Final;
float Angle_Z_Final;
extern signed short  imu660rb_acc_y, imu660rb_acc_z;            //加速度传感器原始数据
extern signed short imu660rb_gyro_x;         //陀螺仪原始数据
